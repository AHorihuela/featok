...
export default function SwipePage({ params }: PageProps) {
  ...
  const fetchIdeas = async () => {
    try {
      const response = await fetch(`/api/ideas/group/${params.id}`);
      if (!response.ok) {
  ...
  const handleVote = async (type: VoteType) => {
    ...
    try {
      const response = await fetch(`/api/ideas/${currentIdea.shareableId}/vote`, {
        method: 'POST',
        headers: {
  ...
  const handleDrag = (event: PointerEvent | TouchEvent | MouseEvent, info: PanInfo) => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">You&apos;ve voted on all the ideas in this group.</p>
          <div className="mt-8 space-y-4">
            <h2 className="text-xl font-semibold">Final Results:</h2>
            {ideas.map(idea => (
              <div key={idea._id} className="bg-white dark:bg-gray-800 rounded-lg p-4 shadow">
      ...
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="relative h-[60vh] flex items-center justify-center">
          ...
          <div className="absolute inset-0 pointer-events-none">
            <div className={`absolute inset-x-0 top-0 h-1/3 flex items-center justify-center transition-opacity ${
            ...
            <div className={`absolute right-0 inset-y-0 w-1/3 flex items-center justify-center transition-opacity ${
            ...
            <div className={`absolute left-0 inset-y-0 w-1/3 flex items-center justify-center transition-opacity ${
          ...
          >
            <div className={`bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 space-y-4 transition-colors ${
              swipeDirection === 'superLike' ? 'bg-pink-50 dark:bg-pink-900' :
              swipeDirection === 'up' ? 'bg-green-50 dark:bg-green-900' :
              swipeDirection === 'neutral' ? 'bg-gray-50 dark:bg-gray-900' : ''
            }`}>
              <h2 className="text-2xl font-bold">{currentIdea.title}</h2>
              <p className="text-gray-600 dark:text-gray-300">{currentIdea.description}</p>
